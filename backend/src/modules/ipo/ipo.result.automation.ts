...
export async function checkIpoResults() {
  ...
  const iposToCheck = unpublishedIpos.filter(
    (ipo) => ipo.issueCloseDate && ipo.issueCloseDate < now,
  );
  ...
  if (!refUsername) {
    console.error(
      "[ResultAutomation] REFERENCE_ACCOUNT_USERNAME not set in config.",
    );
  ...
  if (!refAccount) {
    console.error(
      `[ResultAutomation] Reference account ${refUsername} not found.`,
    );
  ...
  try {
    ...
    console.error(
      "[ResultAutomation] Failed to get application report for reference account",
      error,
    );
  ...
  for (const ipo of iposToCheck) {
    console.info(
      `[ResultAutomation] Checking result for IPO: ${ipo.companyName} (${ipo.companyShareId})`,
    );
    ...
    if (!existingApp) {
      console.warn(
        `[ResultAutomation] Reference account did not apply for ${ipo.companyName}. Cannot check result.`,
      );
      continue;
    }
    ...
    try {
      ...
      const isPublished =
        detail.statusName === "Alloted" || detail.statusName === "Not Alloted";

      if (isPublished) {
        console.info(
          `[ResultAutomation] 🚀 Result is PUBLISHED for ${ipo.companyName}!`,
        );
        await processPublishedIpo(ipo);
      } else {
        console.info(
          `[ResultAutomation] Result not yet published for ${ipo.companyName} (Current Status: ${detail.statusName})`,
        );
      }
    } catch (err) {
      console.error(
        `[ResultAutomation] Error checking detail for ${ipo.companyName}:`,
        err,
      );
    }
  }
...
async function processPublishedIpo(ipo: any) {
  ...
  const userLiveApps: Map<string, LiveApplicationStatus[]> = new Map();
  ...
  for (const account of allAccounts) {
    ...
    try {
      ...
      let finalStatus: LiveApplicationStatus["status"] = "not_applied";
      ...
      if (accApp) {
        ...
        if (
          ...
          accApp.statusName === "BLOCK_FAILED"
        ) {
          finalStatus = "error";
      ...
      console.error(
        `[ResultAutomation] Failed to fetch result for account ${account.username}`,
        err,
      );
    }
  ...
  console.info(
    `[ResultAutomation] Successfully processed and notified for ${ipo.companyName}`,
  );
}
